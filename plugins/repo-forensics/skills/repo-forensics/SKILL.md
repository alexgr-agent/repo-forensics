...
| **git_config** | Executable git configuration (Beltdown class): shipped `.git` dirs (filesystem incl. ignored dependency roots, + archives), armed `.git/config` exec keys (incl. `filter.*` checkout exec, `gpg.program`, `diff.external`, `core.askpass`), hostile `gitdir:` pointers with worktree-proof logic, `.gitmodules` `update = !command`, staged config-write + rename/copy-to-`.git` plant with one-hop variable resolution | skill + full |
