...
def _resolve_tool_version():
    ...
    for rel in (".claude-plugin/plugin.json", ".kimi-plugin/plugin.json",
        ...
        try:
            ...
            continue
    # Never a real release version -- if this fires, no manifest was found at
    # all (a standalone skill copy), and hardcoding one here would just be
    # the next value to go stale.
    return "unknown"
